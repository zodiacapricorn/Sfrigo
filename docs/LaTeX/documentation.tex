...
\fancyhf{}
\fancyhead{}
\fancyfoot[C]{\thepage} % Inserisce il numero al centro (Center)
\fancypagestyle{plain}{%
  \fancyhf{}
  \fancyhead{}
  \fancyfoot[C]{\thepage} % Al centro anche nelle pagine di inizio capitolo
}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{add-item.png}
  \caption{POST /api/v1/fridges/:id/items}
  \label{fig:api_add}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{delete-item.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{get-frigos.png}
  \caption{GET /api/v1/fridges}
    \label{fig:api_frigos}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{get-items.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{login-firebase.png}
...
\begin{enumerate}
  ...
  \item L'utente visualizza le ricette proposte e sceglie quella desiderata.
  \item L'utente clicca su \textit{``Usa questa ricetta''}.
  \item Il sistema salva la ricetta in MongoDB e successivamente rimuove dall'inventario condiviso gli ingredienti selezionati. (E' stato inserito un bottone, che permette di vedere le ricette condivise passate, affianco al bottone \textit{Ricetta AI})
\end{enumerate}
...
\begin{ucbox}
\textbf{Descrizione:} Il sistema calcola e visualizza la distribuzione equa del contributo alimentare tra i membri del frigorifero condiviso. L'algoritmo considera il valore effettivo degli alimenti inseriti da ciascun utente presi da MongoDB e calcola quante risorse ciascun membro ha messo a disposizione della collettività, esprimendo il risultato in termini percentuali.
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{\textwidth}
    \centering
    \includegraphics[width=0.95\linewidth, height=0.28\textheight, keepaspectratio]{4.PNG}
