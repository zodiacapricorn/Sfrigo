...
function RecipeModal({ ingredients, currentUserId, fridgeId, onClose, onIngredientsUsed }) {
    ...
    const [error, setError] = useState("");
    const [usingIdx, setUsingIdx] = useState(null); // ricetta in corso di utilizzo
    const [usedIdx, setUsedIdx] = useState(null); // ricetta già usata (feedback)
    ...
    // Risolve il nome di un ingrediente della ricetta al corrispondente item nel frigo
        ...
        <ModalBase onClose={onClose} maxWidth={500}>
            ...
            {recipes && (
                <>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 1200, overflowY: "auto" }}>
                        {recipes.map((r, i) => {
                                ...
                                <div key={i} style={{ border: "1.5px solid rgba(45,74,45,0.1)", borderRadius: 14, overflow: "hidden" }}>
                                    ...
                                    <div style={{ padding: "10px 16px 14px" }}>
                                        ...
                                        <div style={{ marginTop: 12 }}>
                                            {usedIdx === i ? (
                                                <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 14px", background: "rgba(200,224,110,0.18)", borderRadius: 10, color: "#2D4A2D", fontSize: "0.82rem", fontWeight: 500 }}>
                                                    <Check size={13} strokeWidth={2.5} /> Ricetta utilizzata — ingredienti rimossi
                                                </div>
                                            ) : (
                                                <button
                                                    type="button"
                                                    ...
                                                    style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "8px 16px", background: usingIdx === i ? "rgba(45,74,45,0.1)" : "var(--forest)", color: usingIdx === i ? "var(--mid)" : "var(--lime)", fontFamily: "'DM Sans',sans-serif", fontSize: "0.82rem", fontWeight: 500, border: "none", borderRadius: 100, cursor: usingIdx === i || usedIdx !== null ? "not-allowed" : "pointer", opacity: usedIdx !== null && usedIdx !== i ? 0.4 : 1, transition: "all 0.2s" }}
                                                    onMouseEnter={e => { if (usingIdx !== i && usedIdx === null) e.currentTarget.style.background = "var(--moss)"; }}
                                                    onMouseLeave={e => { if (usingIdx !== i && usedIdx === null) e.currentTarget.style.background = "var(--forest)"; }}
                                                >
