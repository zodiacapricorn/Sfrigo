    ...
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box">
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
          <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:"1.45rem",fontWeight:700,color:"var(--forest)"}}>Aggiungi Ingrediente</h3>
        ...
        <form onSubmit={submit}>
          ...
          <div style={{display:"flex",gap:9,marginTop:20}}>
            ...
            ><Plus size={14} strokeWidth={2}/> Aggiungi Ingrediente</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box" style={{maxWidth:370}}>
        <div style={{textAlign:"center"}}>
          ...
          <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:"1.3rem",fontWeight:700,color:"var(--forest)",marginBottom:9}}>Elimina Ingrediente</h3>
...
function IngRow({ ing, delay, isOpen, onToggle, onDelete }) {
  ...
  const details = [
    {label:"Ingrediente",    val:ing.name},
      ...
      {isOpen && (
        <div className="ing-detail">
          ...
          <div style={{marginTop:14,display:"flex",justifyContent:"flex-end"}}>
            ...
            >
              <Trash2 size={12} strokeWidth={1.75}/> Elimina Ingrediente
            </button>
          </div>
        </div>
      )}
    ...
    <>
      ...
      <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(245,240,232,0.92)",backdropFilter:"blur(18px)",borderBottom:"1px solid rgba(45,74,45,0.08)",padding:"0 clamp(14px,4vw,44px)",height:62,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
        ...
        <button className="btn-primary" onClick={()=>setShowAdd(true)}>
          <Plus size={14} strokeWidth={2.25}/> Aggiungi Ingrediente
        </button>
      </header>
      ...
      <div className="page-grid">
        ...
        <section>
          <div className="fade-up" style={{marginBottom:20}}>
            ...
            <p style={{color:"var(--mid)",fontSize:"0.82rem",marginTop:4}}>{ingredients.length} {ingredients.length===1?"Ingrediente":"alimenti"} · clicca per i dettagli</p>
          </div>
          ...
          <div className="col-headers-row" style={{marginBottom:7}}>
            {["Ingrediente","Quantità","Scadenza",""].map((h,i)=>(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {ingredients.length===0 ? (
              <div style={{padding:"52px 24px",textAlign:"center",border:"1.5px dashed rgba(45,74,45,0.18)",borderRadius:14}}>
                ...
                <p style={{color:"var(--mid)",fontSize:"0.83rem",marginBottom:16}}>Aggiungi il primo Ingrediente per iniziare.</p>
                <button className="btn-primary" onClick={()=>setShowAdd(true)}><Plus size={13} strokeWidth={2}/> Aggiungi Ingrediente</button>
              </div>
            ) : (
